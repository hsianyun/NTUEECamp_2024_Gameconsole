...
const int ULT_CODE[ULT_NUM][ULT_LEN] = {{0, 2, 3, 4, 1},
...
void handleUltimateSkill(void){
    ...
    static int ultCount[ULT_NUM] = {0}; // Array to store current step of each ultimate skill
    static clock_t lastTimeKeyPress = 0;
    for(int i = 0; i < ULT_NUM; i++){
        if(1000 * (clock() - lastTimeKeyPress) > ULT_DELAY_MS * CLOCKS_PER_SEC){
            ...
            for(int j = 0; j < NUM_OF_ROW*NUM_OF_COL; j++){
    ...
    for(int i = 0; i < NUM_OF_ROW*NUM_OF_COL; i++){
...
void detectKeyboard(void){
    ...
    for(int i = 0; i < NUM_OF_ROW; i++){
        ...
        for(int j = 0; j < NUM_OF_COL; j++){
            if(bouncing[i*NUM_OF_COL + j]){
                if(curr_key_state[i*NUM_OF_COL + j] != gpio_get_level(COLUMN[j])){
                    // Bounce back, treat as no bounce
                    bouncing[i*NUM_OF_COL + j] = 0;
                    curr_key_state[i*NUM_OF_COL + j] = last_key_state[i*NUM_OF_COL + j];
                }
                else if(1000 * (currentTime - timeOfChange[i*NUM_OF_COL + j]) < BOUNCE_DELAY_MS * CLOCKS_PER_SEC){
                    // Waiting for bouncing ends
                    curr_key_state[i*NUM_OF_COL + j] = last_key_state[i*NUM_OF_COL + j];
                }
                else{
                    // Bouncing ends
                ...
                curr_key_state[i*NUM_OF_COL + j] = last_key_state[i*NUM_OF_COL + j];
                if(curr_key_state[i*NUM_OF_COL + j] != (!gpio_get_level(COLUMN[j]))){
                    // input changes, start bouncing delay
                    bouncing[i*NUM_OF_COL + j] = 1;
                }
            }
        }
